package com.github.jmetzz.laboratory.concurrency.standalone.synchronizationExample;

import com.github.jmetzz.laboratory.concurrency.standalone.synchronizationExample.runnables.*;
...
public class AppThreadController {
    ...
    private static void atomicCounterExample() throws InterruptedException {
        System.out.println(" --------Atomic counter Example---------- ");
        Thread[] threads = new Thread[5];
        for (int i = 0; i < threads.length; i++) {
            threads[i] = new Thread(new AtomicCounterRunnable(i, 2000), "thread-" + i);
            threads[i].start();
        }
        for (int i = 0; i < threads.length; i++) {
            threads[i].join();
