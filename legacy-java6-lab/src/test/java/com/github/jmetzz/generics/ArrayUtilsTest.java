...
public class ArrayUtilsTest {
    ...
    public void testMinmax1() throws Exception {
        Pair<GregorianCalendar> mm = ArrayUtils.minmax1(birthdays);
...
//        Pair<Number> pNumber = ArrayUtils.minmax1(nArray);  // -- Compiler Error: Number is not Comparable
        Pair<Integer> pInt = ArrayUtils.minmax1(iArray);
        ...
        // minmax1 method return a Pair<T> in which <T extends Comparable>.
        // Both Integer and Double implement Comparable interface
        // but the is-a relationship between Pair<Integer> and Pair<Double>
        // doesn't hold.
        // Therefore, the minmax1 method returns a Pair<Integer>
        // and not a Pair<Double>
        // Pair<Double> pDouble = ArrayUtils.minmax1(iArray);
    ...
    public void testMinmax2() throws Exception {
        TwoDPoint[] points = {
                new TwoDPoint(new Pair(1, 1)),
                new TwoDPoint(new Pair(2, 3)),
                new TwoDPoint(new Pair(4, 5)),
                new TwoDPoint(new Pair(6, 2)),
                new TwoDPoint(new Pair(6, 8)),
                new TwoDPoint(new Pair(2, 3))
        };
        Pair<TwoDPoint> pair = ArrayUtils.minmax2(points);
    ...
    public void testMinmax3() throws Exception {
        ...
        * Pair<GregorianCalendar> mm = ArrayAlg.minmax2(birthdays);   -- Compilation Error
        * */

        Pair<GregorianCalendar> mm = ArrayUtils.minmax3(birthdays);
        assertThat(mm.getLeft()).isEqualTo(birthdays[1]);
        assertThat(mm.getRight()).isEqualTo(birthdays[3]);
    ...
    public void testBoundedPair() {
        /*
        * BoundedPair accepts argument types that are
        * both Comparable and Serializable.
        * Therefore, we can not instantiate a BoundedPair
        * with Point, since Point is Comparable<Point>
        * but not Serializable.
        *
        * BoundedPair<Point> ibPair = new BoundedPair<Point>(new Point(), new Point()); // -- Compile-time error
        *
        * Integer, on the other hand, is both Comparable and Serializable
        * and thus can be used to instantiate a BoundedPair
        * */
        BoundedPair<Integer> ibPair = new BoundedPair<Integer>(3, 5);
    }
